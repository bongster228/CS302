...
nodeType<myType>* findEntry(myType k);              // Search the skip list for a given key, k, and return a node in the base layer with
...
template<class myType>
unsigned int skipList<myType>::getWordCount() const{
    return totalCount;
}
...
template<class myType>
nodeType<myType>* skipList<myType>::findEntry(myType k){
