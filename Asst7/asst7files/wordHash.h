...
private:
    ...
    static const double loadFactoro = 0.65;
    static const int initialHashSize = 30011;

    void insert(std::string, unsigned int);                     // Insert the passed item ino the hash table. If the hash
...
public:
    ...
    void insert(std::string);                                   // Increment the word count if the item is
                                                                // already in the tabale and if not, use the private
                                                                // function to inser the inter into the table.
    ...
    void getMaxNode (unsigned int&, string&) const;             // Search for the word string associated with the largest count and
